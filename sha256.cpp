...
#ifndef _mingw_      // not available on mingw, fine without
#ifndef _MSC_VER
#include <endian.h>
#endif
#endif
...
void SHA256::processBuffer()
{
    ...
    *addLength   =  msgBits        & 0xFF;
    ...
    // process blocks
    processBlock(m_buffer);
    // flowed over into a second block ?words[i] =      input[i];
    if (paddedLength > BlockSize)
        processBlock(extra);
}
...
const unsigned char* SHA256::getBinary()
{
    // save old hash if buffer is partially filled
    uint32_t oldHash[HashValues];
    for (int i = 0; i < HashValues; i++)
    ...
    for (int i = 0; i < HashValues; i++)
    {
        buf[offset++] = (m_hash[i] >> 24) & 255;
        buf[offset++] = (m_hash[i] >> 16) & 255;
        buf[offset++] = (m_hash[i] >> 8) & 255;
        buf[offset++] =  m_hash[i] & 255;
...
/**  Hash-based message authentication code
  ...
  @return pointer to binary digest, 64 bytes long, valid until
                    next call to hmac or getBinary()
...
*/
const unsigned char * SHA256::hmac(
                                    const unsigned char * key,
                                    int keylen,
                                    const std::string msg )
{

    const int blocksize_bytes = 512 / 8;
    const int outputsize_bytes = 256 / 8;
    unsigned char keybuf[ blocksize_bytes ];
    std::memset( keybuf, 0, blocksize_bytes);
    if( keylen > blocksize_bytes )
    {
        SHA256 sha256;
        sha256.add(key,keylen);
        memcpy( keybuf, sha256.getBinary(), outputsize_bytes );
    }
    else
    {
    ...
    char o_key_pad[ blocksize_bytes];
    char i_key_pad[ blocksize_bytes];

    for( int k = 0; k < blocksize_bytes; k++ )
    {
        o_key_pad[ k ] = keybuf[k] ^ 0x5c;
        i_key_pad[ k ] = keybuf[k] ^ 0x36;
    }

    SHA256 sha256_i;
    sha256_i.add( i_key_pad, blocksize_bytes );
    sha256_i.add( msg.c_str(), msg.length() );

    SHA256 sha256_o;
    sha256_o.add( o_key_pad, blocksize_bytes );
    sha256_o.add( (void*)sha256_i.getBinary(), outputsize_bytes );

    return sha256_o.getBinary();
}
